...
SuperBob is an aggregation of open-source skills. It does not claim ownership of them —
each skill was created by its respective author and remains under that author's own
license.

SuperBob's own code — the installer, the profile system, and the mission-control router
— is licensed **MIT**. That covers only the original code, not the bundled skills.
...
Original skills bundled here (`mission-control`, `caveman-debug`, and the profile
system) are MIT.
