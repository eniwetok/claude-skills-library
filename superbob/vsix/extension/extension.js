...
function managerHtml() {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>
    ...
    .tplhint{font-size:11.5px;color:var(--vscode-descriptionForeground);min-height:15px;margin:2px 0 4px}
  </style></head><body>
    ...
    <div id="editor">
      ...
      <div id="tplHint" class="tplhint"></div>
    ...
    <script>(function(){
      ...
      const TPL={ skill:[
          {n:'Full skill', d:'Most skills: written instructions Bob follows to produce something.', t:'---\\nname: NAME\\ndescription: <What it does, in third person.> Use when <the situations that should trigger it>, and proactively when <a condition worth surfacing it for>.\\nlicense: MIT\\nmetadata:\\n  category: <category>\\n  tags: [<tag>, <tag>]\\n---\\n\\n# NAME\\n\\n## Overview\\nWhat this skill does and the concrete outcome it produces, in two or three sentences.\\n\\n## When to use\\n- <situation the user is in>\\n- <another situation>\\n\\n## Instructions\\n1. <first step>\\n2. To keep this file lean, put deep detail in references/ and say: read references/<topic>.md for the rules.\\n3. To run logic, call a bundled script: python3 scripts/<tool>.py <args>  (add it under Attached files).\\n4. Produce <the output>.\\n\\n## Examples\\n- Input: <what the user provides>\\n  Output: <what the skill returns>\\n\\n## Resources\\n- references/<topic>.md - deeper guidance, loaded on demand\\n- scripts/<tool>.py - run for <purpose>\\n'},
          {n:'With scripts (progressive)', d:'When a step runs a script, or the skill carries big reference docs.', t:'---\\nname: NAME\\ndescription: <What it does>. Use when <trigger>. Bundles helper scripts and reference docs so the main file stays small.\\nlicense: MIT\\n---\\n\\n# NAME\\n\\n## Overview\\n<one paragraph on the outcome>\\n\\n## Instructions\\n1. Gather inputs: <...>\\n2. Run the extractor: python3 scripts/extract.py <input>   (attach extract.py under Attached files).\\n3. For the rules and edge cases, read references/rules.md (attach it too) instead of inlining them here.\\n4. Produce <output>.\\n\\n## Progressive disclosure\\nKeep SKILL.md short. Put long tables, rules, and examples in references/ and load them only when needed. Put runnable logic in scripts/. This is the pattern the best skills use.\\n'},
          {n:'Review skill', d:'When the job is to audit or validate something and report fixes.', t:'---\\nname: NAME\\ndescription: Reviews <artifact> against <standard> and reports concrete fixes. Use when someone asks to check, audit, or validate <artifact>.\\n---\\n\\n# NAME\\n\\n## When to use\\nWhen the user asks to review, audit, or validate <artifact>.\\n\\n## Checklist\\nGo through each item and report pass or fail with a one-line reason:\\n- [ ] <criterion 1>\\n- [ ] <criterion 2>\\n- [ ] <criterion 3>\\n\\n## Output\\nReturn the failures first, each with the smallest fix that resolves it. End with an overall pass or fail.\\n'} ],
        agent:[
          {n:'Full subagent', d:'A complete subagent: example trigger, boundaries, process, output.', t:'---\\nname: NAME\\ndescription: <What this agent is good at, in one line.> Use this agent when <the specific trigger>. <example>Context: <situation>. user: "<request>". assistant: "<one line on why this agent is dispatched>"</example>\\nwhenToUse: Delegate when the task is to <the specific job>.\\ngroups: [read, edit, command]\\nmodel: inherit\\n---\\nYou are <role>, a focused specialist.\\n\\nResponsibilities\\n- <what you do, 1>\\n- <what you do, 2>\\n\\nBoundaries (do not)\\n- <what you must never do>\\n- Stay within this task; do not expand scope on your own.\\n\\nProcess\\n1. <first step>\\n2. <step>\\n3. <step>\\n\\nOutput\\nReturn <format>, most important first. Be concise and state what you verified.\\n'},
          {n:'Reviewer', d:'A subagent that reviews or audits and reports the issues that matter.', t:'---\\nname: NAME\\ndescription: Reviews <artifact> and reports the issues that matter. Use this agent when the task is to review, audit, or critique <artifact>. <example>Context: a change is ready. user: "review this". assistant: "dispatching the reviewer to check it before merge"</example>\\nwhenToUse: Delegate when the task is to review or audit <artifact>.\\ngroups: [read]\\nmodel: inherit\\n---\\nYou review <artifact>.\\n\\nProcess\\n1. Read the artifact and the relevant standard.\\n2. Check for correctness, edge cases, and safety.\\n\\nOutput\\nReturn a short list of concrete issues, most important first, each with the smallest fix. Do not rewrite the whole thing.\\n'},
          {n:'Builder', d:'A subagent that builds or scaffolds something to spec.', t:'---\\nname: NAME\\ndescription: Builds <artifact> to spec. Use this agent when the task is to create, scaffold, or wire up <artifact>. <example>Context: a spec exists. user: "build the endpoint". assistant: "handing this to the builder to scaffold it to spec"</example>\\nwhenToUse: Delegate when the task is to build or scaffold <artifact>.\\ngroups: [read, edit, command]\\nmodel: inherit\\n---\\nYou build <artifact> to spec.\\n\\nProcess\\n1. Confirm the spec and the interface.\\n2. Scaffold the smallest correct version, typed and testable.\\n3. Note anything that needs a credential or tool so it can be set up as a Requirement.\\n\\nOutput\\nReturn the files and a one-line summary of what to run next.\\n'} ] };
      function renderTemplates(){ const c=$('tplBtns'); c.innerHTML=''; const hint=$('tplHint'); if(hint) hint.textContent=''; (TPL[kind]||[]).forEach(t=>{ const b=document.createElement('button'); b.className='ghost tpl'; b.textContent=t.n; b.title=t.d||''; b.onmouseenter=()=>{ if(hint) hint.textContent=t.d||''; }; b.onclick=()=>{ const nm=slug($('ename').value)||(kind==='agent'?'my-agent':'my-skill'); $('ebody').value=t.t.replace(/NAME/g,nm); touched=true; validate(); if(hint) hint.textContent='Inserted "'+t.n+'". '+(t.d||''); }; c.appendChild(b); }); }
...
<script nonce="${nonce}">
  ...
  let S=null; let sel=new Set(); let pendingScrollKit=null;
  vscode.postMessage({type:'ready'});
  window.addEventListener('message', e=>{ if(e.data.type==='state'){ S=e.data; render();
    if(pendingScrollKit){ const k=pendingScrollKit; pendingScrollKit=null; setTimeout(()=>{ const el=document.querySelector('.kitsw[data-kit="'+k+'"]'); const card=el&&el.closest?el.closest('.mode'):null; if(card) card.scrollIntoView({block:'nearest',behavior:'smooth'}); },30); }
  }});
